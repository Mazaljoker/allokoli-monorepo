  ...
  .then(() => {
    ...
    app.post("/api/agent", async (req, res) => {
      ...
      try {
        ...
        return res.status(500).json({
          error: "Erreur achat/configuration numéro Twilio",
          details: err.message,
        });
      }
      ...
      try {
        ...
        return res.status(500).json({
          error: "Erreur update agent phone_number",
          details: err.message,
        });
      }
      ...
      return res
        .status(201)
        .json({ id: agentId, phone_number: purchasedNumber.phoneNumber });
    });
      ...
      async (req, res) => {
        try {
          ...
          const { data: uploadData, error: uploadError } =
            await supabase.storage.from("audios").upload(audioPath, mp3Stream, {
          ...
          if (uploadError) {
            ...
            return res.status(500).json({
              error: "Erreur upload audio Supabase",
    ...
    app.post("/api/tts/speak", async (req, res) => {
      ...
      try {
        ...
        res
          .status(500)
          .json({ error: "Gateway error", details: (e as Error).message });
      }
    });
